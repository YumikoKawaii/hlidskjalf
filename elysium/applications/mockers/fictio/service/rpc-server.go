...
func (s *Server) Register(grpcServer ...interface{}) error {
	for _, srv := range grpcServer {
		...
		case pb.FictioServiceServer:
			pb.RegisterFictioServiceServer(s.gRPC, _srv)
			if err := pb.RegisterFictioServiceHandlerFromEndpoint(context.Background(), s.mux, s.cfg.GRPC.String(), []grpc.DialOption{grpc.WithTransportCredentials(insecure.NewCredentials())}); err != nil {
				return err
			}
		default:
