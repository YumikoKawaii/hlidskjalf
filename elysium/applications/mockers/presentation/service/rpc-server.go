...
func (s *Server) Register(grpcServer ...interface{}) error {
	for _, srv := range grpcServer {
		...
		case pb.PresentationServiceServer:
			pb.RegisterPresentationServiceServer(s.gRPC, _srv)
			if err := pb.RegisterPresentationServiceHandlerFromEndpoint(context.Background(), s.mux, s.cfg.GRPC.String(), []grpc.DialOption{grpc.WithTransportCredentials(insecure.NewCredentials())}); err != nil {
				return err
			}
		default:
...
func (s *Server) Serve() error {
	...
	go func() {
		logrus.Infof("[Center] - HTTP: %d", s.cfg.HTTP.Port)
		logrus.Infof("[Center] - GRPC: %d", s.cfg.GRPC.Port)
		logrus.Info("[Center] - Serving")
