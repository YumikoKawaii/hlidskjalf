...
func (c *httpImpl) Fictio(ctx context.Context) (Response, error) {

	start := time.Now()
	requestUrl := fmt.Sprintf("%s/api/v1/fictio", c.host)
	req, _ := http.NewRequest(http.MethodGet, requestUrl, nil)
	respData, err := c.client.Do(req)
	logrus.Infof("[Fictio] - Request: %d ms", time.Since(start).Milliseconds())
	if err != nil {
		return Response{}, err
	}
