...
func (c *httpImpl) Mimic(ctx context.Context) (Response, error) {

	start := time.Now()
	requestUrl := fmt.Sprintf("%s/api/v1/mimic", c.host)
	req, _ := http.NewRequest(http.MethodGet, requestUrl, nil)
	respData, err := c.client.Do(req)
	logrus.Infof("[Mimic] - Request: %d ms", time.Since(start).Milliseconds())
	if err != nil {
		return Response{}, err
	}
