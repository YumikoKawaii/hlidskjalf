...
func (c *httpImpl) Echo(ctx context.Context) (Response, error) {

	start := time.Now()
	requestUrl := fmt.Sprintf("%s/api/v1/echo", c.host)
	req, _ := http.NewRequest(http.MethodGet, requestUrl, nil)
	respData, err := c.client.Do(req)
	logrus.Infof("[Echo] - Request: %d ms", time.Since(start).Milliseconds())
	if err != nil {
		return Response{}, err
	}
