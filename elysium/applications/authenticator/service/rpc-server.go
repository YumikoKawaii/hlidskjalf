...
func (s *Server) Register(grpcServer ...interface{}) error {
	for _, srv := range grpcServer {
		...
		case pb.AuthenticatorServer:
			pb.RegisterAuthenticatorServer(s.gRPC, _srv)
			if err := pb.RegisterAuthenticatorHandlerFromEndpoint(context.Background(), s.mux, s.cfg.GRPC.String(), []grpc.DialOption{grpc.WithTransportCredentials(insecure.NewCredentials())}); err != nil {
				return err
			}
		default:
