...
func (s *Server) Register(grpcServer ...interface{}) error {
	for _, srv := range grpcServer {
		...
		case pb.InteractionServiceServer:
			pb.RegisterInteractionServiceServer(s.gRPC, _srv)
			if err := pb.RegisterInteractionServiceHandlerFromEndpoint(context.Background(), s.mux, s.cfg.GRPC.String(), []grpc.DialOption{grpc.WithTransportCredentials(insecure.NewCredentials())}); err != nil {
				return err
			}
		default:
...
func (s *Server) Serve() error {
	...
	go func() {
		...
		logrus.Infof("[Interaction] - HTTP: %d", s.cfg.HTTP.Port)
		logrus.Infof("[Interaction] - GRPC: %d", s.cfg.GRPC.Port)
		logrus.Info("[Interaction] - Serving")
