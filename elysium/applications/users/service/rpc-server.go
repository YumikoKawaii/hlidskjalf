...
func (s *Server) Register(grpcServer ...interface{}) error {
	for _, srv := range grpcServer {
		...
		case pb.UserServiceServer:
			pb.RegisterUserServiceServer(s.gRPC, _srv)
			if err := pb.RegisterUserServiceHandlerFromEndpoint(context.Background(), s.mux, s.cfg.GRPC.String(), []grpc.DialOption{grpc.WithTransportCredentials(insecure.NewCredentials())}); err != nil {
				return err
			}
		default:
...
func (s *Server) Serve() error {
	...
	go func() {
		...
		logrus.Infof("[User] - HTTP: %d", s.cfg.HTTP.Port)
		logrus.Infof("[User] - GRPC: %d", s.cfg.GRPC.Port)
		logrus.Info("[User] - Serving")
